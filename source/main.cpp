...
void Start()
{
	auto *Scene = new BigNgine::Scene([](BigNgine::Scene *Scene)
									  {
...
//										  adding a 20 pixel margin bc we have some precision issues with follow camera behaviour
										  auto *Sky = new BigNgine::Entity(BigNgine::Vector2(-610, -410), 0.0f,
																		   BigNgine::Vector2((float) Game::width + 20,
...
//										  sky follow camera :)
										  ...
										  auto *flipTextureOnMovement = new Input::Callback([Player](int key, int scancode, int mods)
																					{
																						if(key == BIGNGINE_KEY_A) {
																							Player->GetBehaviours<BigNgine::TextureRendererBehaviour>()[0]->xFlipped = false;
																						} else if(key == BIGNGINE_KEY_D) {
